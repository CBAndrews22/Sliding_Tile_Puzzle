...
bool MisplacedTile::isVisited(node* curNode){
...
void MisplacedTile::push(node* newNode){
    ...
    if(!isVisited(newNode)){
        ...
        while(index > 0 && heap[index]->misplacedVal < heap[parentIndex]->misplacedVal){ //while index is not the rood & The new node value < Parent node value
...
void MisplacedTile::swap(int index1, int index2){
    node* temp = heap[index2];
    heap[index2] = heap[index1];
    heap[index1] = temp;
};

node* MisplacedTile::pop(){
    node* temp = heap[0];
    int curIndex = 0;
    int compareIndex;
    int child1, child2;
    ...
    while(!heap.empty()){
        ...
        //If curIndex has two children
            ...
            if(heap[child1]->misplacedVal < heap[child2]->misplacedVal){ 
                compareIndex = child1;
            }
        ...
        if(heap[curIndex]->misplacedVal > heap[compareIndex]->misplacedVal){ 
